...
# FIX DEFINITIVO: Aggiungiamo un User-Agent da browser per evitare blocchi 403
    ...
    if st.button("🔁 Controlla inbox (Temp-Mail.org)"):
        with st.spinner("Recupero messaggi..."):
            try:
                ...
                # FIX CRITICO: Aggiungiamo l'header User-Agent alla richiesta
                r = requests.get(f"https://web2.temp-mail.org/messages/{md5_hash}", headers=USER_AGENT_HEADER)
                r.raise_for_status()
                messages = r.json()
                if not messages: st.info("📭 Nessun messaggio trovato."); return
                st.success(f"Trovati {len(messages)} messaggi.")
